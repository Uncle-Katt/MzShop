...
<body>
<h1>Quản lý khối lượng</h1>
<form method="post" action="/khoi-luong/add">
    <div>
        <label>Tên Danh Mục</label>
        <input type="text" name="tenKhoiLuong" value="${data1.tenKhoiLuong}">
    </div>
    <div>
        <div>
...
<table class="table" border="1">
    ...
    <c:forEach items="${dskl}" var="kl" varStatus="i">
        <tr>
            <td scope="row">${i.index + 1}</td>
            <td>${kl.tenKhoiLuong}</td>
            <td>${kl.trangThai == 1 ? "Đang hoạt động" : "Ngừng hoạt động"}</td> <!-- Thay sv thành sp -->
        </tr>
    </c:forEach>
</table>
